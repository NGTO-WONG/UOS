...
namespace Watermelon
{
    public partial class TextureSmartImporter : EditorWindow
    {
        ...
        private IEnumerator InitEnumerator(bool initAll, params TextureCase[] texturesCase)
        {
            if (selectedPresetID != -1)
            {
                for (int textureIndex = 0; textureIndex < texturesCase.Length; textureIndex++)
                {
                    ...
                    textureImporter.spritePackingTag = presets[selectedPresetID].packingTag;
        ...
        private class TexturePostprocessor : AssetPostprocessor
        {
            public void OnPreprocessTexture()
            {
                if (CustomProjectPreferences.ShowAudioImportWindow)
                {
                    //Check if texture meta file exists (if not - it means, texture is imported for the first time)
                    if (File.Exists(AssetDatabase.GetTextMetaFilePathFromAssetPath(assetImporter.assetPath.ToLower())))
                        return;

                    TextureImporter textureImporter = (TextureImporter)assetImporter;
                    if (textureImporter != null)
                    {
                        TextureSmartImporter.Open(new TextureCase(null, textureImporter));
                    }
                }
            }
        }
