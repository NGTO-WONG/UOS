    ...
    public class FSR2_POST_PROCESS_PASS : CustomPostProcessVolumeComponent, IPostProcessComponent
    {
        ...
        public override void Render(CommandBuffer cmd, HDCamera camera, RTHandle source, RTHandle destination) {
            if(Application.isPlaying) {
                ...
                if(Shader.GetGlobalTexture(depthTexturePropertyID).graphicsFormat != UnityEngine.Experimental.Rendering.GraphicsFormat.None) {
                    m_hdrp.OnDepthFormat(false);
                } else {
                    m_hdrp.OnDepthFormat(true);
                }
