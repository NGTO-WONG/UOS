...
public class ToolBarEditor
{
    ...
    private static void DropDown()
    {
        ...
        async Task T()
        {
            ...
            if (tryToCheckOutIndex == oldIndex || tryToCheckOutIndex == -1)
            {
                return;
            }
            //询问是否切换
...
public static class GitHelper
{
    ...
    /// </summary>
    /// <returns></returns>
    public static async Task<bool> CheckCommit(string currentBranch)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public static async Task<(List<string>, string)> OpenCommitWindow()
    {
        ...
        while (window.State == "Idle")
        {
            // 短暂等待后继续检查
            await Task.Delay(100); // 这里的延迟可以根据实际情况调整
        }
    ...
    public static async Task<bool> CommitAndPush(List<string> files, string message)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public static async Task<List<string>> GetModifiedFiles()
    {
    ...
    /// git更新 
    /// </summary>
    public static async Task GitPull()
    {
        string error;
        await RunGitCommand("reset --hard");
        await RunGitCommand("clean -df");
    ...
    /// </summary>
    /// <returns></returns>
    public static async Task<(string[] branches, string currentBranchName)> GetBranchInfo()
    {
        // 获取所有分支名
    ...
    static async Task<(string output, string error)> RunGitCommand(string command)
    {
        ...
        using (Process process = new Process())
        {
            ...
            var output = readOutputTask.Result;
            var error = readErrorTask.Result;
